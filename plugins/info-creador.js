...
let biografia = await conn.fetchStatus('525544876071' +'@s.whatsapp.net').catch(_ => 'Sin Biografía')
...
let name = await conn.getName(who)
  ...
  await sendContactArray(conn, m.chat, [
     [`${nomorown}`, `👑 Propietario`, `𝚃𝙷𝙴 𝙲𝙰𝚁𝙻𝙾𝚂`, dev, correo, `🇲🇽 México` , `${global.yt}`, bio],
[`${conn.user.jid.split('@')[0]}`, `Bot oficial 🍟`, `${packname}`, `📵 No Hacer Spam`, correo, `🇲🇽 México`, md, biobot]
], m)
