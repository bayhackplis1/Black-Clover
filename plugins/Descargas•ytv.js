...
if (!args[0]) return conn.reply(m.chat, `🚩 Te Faltó Un Link De Un Video De Youtube`, m, rcanal)
...
await await conn.sendMessage(m.chat, { video: { url: dl_url }, fileName: `${ttl}.mp4`, mimetype: 'video/mp4', caption: `⚔️ Aquí está tu video de Youtube.`, thumbnail: await fetch(yt.thumbnail) }, { quoted: m })
await m.react(done)
} catch (E1) {
await m.react(error)  
try {  
...
await conn.sendMessage(m.chat, { video: { url: n2 }, fileName: `${n}.mp4`, mimetype: 'video/mp4', caption: `⚔️ Aquí está tu video de Youtube.`, thumbnail: await fetch(n4) }, { quoted: m })
await m.react(done)
} catch (E3) {
await m.react(error)
